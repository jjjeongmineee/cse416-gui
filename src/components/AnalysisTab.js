...
export default class AnalysisTab extends React.Component {
    ...
    render() {
        ...
        return (
            <div role="tabpanel" hidden={selectedTab !== tabIndex} id={`analysisTab${tabIndex}`}
                 aria-labelledby={`analysisTab${tabIndex}`} className='tabPanel'>
                        ...
                        <div className='pValHolder'>
                            {pVals.map((el) =>
                                <div className='paddedProgress' key={el}>
                                    <CircularProgressbar value={100 - el * 100} text={`p=${el}`} minValue={75}
                                                         circleRatio={0.75} styles={buildStyles({
                                        rotation: 0.625,
